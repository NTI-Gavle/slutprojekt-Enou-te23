...
try {
    ...
    if ($roomId) {
        ...
        if ($after > 0) {
            $stmt = $db->prepare("
                SELECT m.id, m.room_id, m.sender_id, m.receiver_id, COALESCE(m.message, m.content) as message, m.attachments, m.created_at, u.display_name as sender_name, u.profile_image as sender_avatar
                FROM messages m
